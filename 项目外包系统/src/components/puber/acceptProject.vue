      ...
      <el-table-column prop="address" label="操作" fixed="right">
        <template slot-scope="scope">
          <el-row>
            ...
            type="primary" 
            icon="el-icon-check" 
            @click="showAcceptProjectDia(scope.row.id)"
            circle></el-button>
          </el-row>
        </template>
      </el-table-column>
...
export default {
    ...
    methods:{
    //根据id验收项目
    showAcceptProjectDia(ProjectId){
        this.$confirm("此操作将验收项目, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
        .then(async () => {
          const res = await this.$http.put(`projects/acceptance/${ProjectId}`);
          console.log(res);
    ...
    //模糊查询项目
    async searchProject() {
      const res = await this.$http.get(`/projects/getProjectByPuberAndPstatus/${this.query}`);
    ...
    const res = await this.$http.get( `projects/getProjectsByPstatus`);
      console.log(res);
